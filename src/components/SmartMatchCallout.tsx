import { Sparkles, Bell, SlidersHorizontal, Mail, ArrowRight, CheckCircle } from 'lucide-react';
    ...
    <section className="bg-white border-t border-slate-100">
      <div className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8 py-16 lg:py-20">
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          ...
          <div>
            <div className="inline-flex items-center gap-2 bg-emerald-50 border border-emerald-200 text-emerald-700 text-[11px] font-semibold px-3 py-1.5 rounded-full mb-5 tracking-wide uppercase">
            ...
            <ul className="space-y-3 mb-8">
              {[
                { icon: <SlidersHorizontal className="w-4 h-4 text-emerald-600" />, text: 'Set your preferences once — niche, audience, location, budget' },
                { icon: <Sparkles className="w-4 h-4 text-emerald-600" />, text: 'Every listing gets a personal match score based on your criteria' },
                { icon: <Bell className="w-4 h-4 text-emerald-600" />, text: 'Configure daily or weekly alert emails with your top-matched slots' },
                { icon: <Mail className="w-4 h-4 text-emerald-600" />, text: 'Digest emails link directly back to live opportunities' },
              ].map((item, i) => (
                <li key={i} className="flex items-start gap-3">
                  <div className="w-7 h-7 bg-emerald-50 border border-emerald-100 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5">
                    {item.icon}
                  </div>
                  ...
                  <div key={card.name} className="bg-white border border-slate-200 rounded-2xl p-3.5 flex items-center gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        <span className="text-[10px] bg-emerald-50 border border-emerald-200 text-emerald-700 font-semibold px-1.5 py-0.5 rounded-md flex-shrink-0">{card.discount}</span>
                      </div>
                      <p className="text-[11px] text-slate-400">{card.pub} · {card.subs} subs</p>
                    </div>
