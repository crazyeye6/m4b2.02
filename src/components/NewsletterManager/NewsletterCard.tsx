import { CreditCard as Edit2, Trash2, Download, Globe, Mic2, Zap, MapPin } from 'lucide-react';
    ...
    <div className="bg-white border border-black/[0.06] rounded-2xl p-4 hover:border-black/[0.12] transition-all">
      <div className="flex items-start gap-3">
        <div className="w-10 h-10 bg-sky-50 border border-sky-100 rounded-xl flex items-center justify-center flex-shrink-0">
          <Mic2 className="w-5 h-5 text-sky-600" />
        </div>
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-3 mt-2.5">
            ...
            {newsletter.primary_geography && (
              <Stat icon={<MapPin className="w-3 h-3" />} value={newsletter.primary_geography} />
            )}
            {newsletter.send_frequency && (
              <Stat icon={<Globe className="w-3 h-3" />} value={newsletter.send_frequency} />
            )}
          </div>
